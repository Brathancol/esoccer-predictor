...
def generar_urls(nombres):
    return [{"nombre": nombre, "url": f"https://esoccerbet.org/fifa-12-minutes/{nombre.lower()}/"} for nombre in nombres]
...
def guardar_en_csv(nombre_jugador, partidos):
    ...
    if not os.path.exists("datos_csv/12m"): os.makedirs("datos_csv/12m")
    archivo_csv = f"datos_csv/12m/{nombre_jugador}.csv"
    ...
    try:
        ...
        print(f"💾 Guardado en Excel: datos_csv/12m/{nombre_jugador}.csv")
...
def registrar_error(error):
    with open("errores_12m.log", "a", encoding="utf-8") as f:
